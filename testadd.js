...
// The uri string must be the connection string for the database
...
async function run() {
  try {
    ...
    const parts = database.collection('brad:myanswer');
...
async function run() {
  try {
    const database = client.db('bjmdb');
    const where2look = database.collection('brad:myanswer');
...
async function run() {
  try {
    const database = client.db('bjmdb');
    const where2put = database.collection('brad:myanswer');
